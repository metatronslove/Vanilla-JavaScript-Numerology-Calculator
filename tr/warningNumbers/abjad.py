...
def numerolog(metin, tablo='tr', outas='tam', shadda=1):
	try:
		...
		traceback.print_exc()
# ----------------------------------------------------------------------
# CSV'leri okuyup tüm verileri toplama
def process_data():
	data = []

	# 1. esmalar.csv işleme
	esmalar_df = pd.read_csv('esmalar.csv', header=None, names=['arapca', 'latin'])
	for _, row in esmalar_df.iterrows():
		arapca = row['arapca'].replace('يا ', '', 1)  # "ya " kaldırma
		latin = row['latin'].replace('Ya ', '', 1)
		
		# 4 varyasyon hesapla
		for method in [1,7]:
			for shadda in [1,2]:
				value = abjad(arapca, method, shadda)
				if shadda == 2:
					valuealone = abjad(arapca, method, 1)
				else:
					valuealone = value
				desc = f"{latin} isminin {'mağribi' if method==7 else ''} ebced değeri (şeddeliler {'tek' if shadda==1 else 'çift'})"
				if shadda == 2:
					if value == valuealone:
						desc = ""
				data.append((value, desc))

	# 2. Ayet dosyalarını işleme
	for file_num in [1,2,3]:
		ayet_df = pd.read_csv(f'Ayet Ebced Değerleri {file_num}.csv')
		for _, row in ayet_df.iterrows():
			arapca = row['Arapça Metin']
			sure = row['Sure Sıra']
			ayet = row['Ayet Sıra']
			...
			for method in [1,7]:
				for shadda in [1,2]:
					...
					if shadda == 2:
						...
						valuealone = value
					desc = f"{sure}. surenin {ayet}. ayetinin {'mağribi' if method==7 else ''} ebced değeri (şeddeliler {'tek' if shadda==1 else 'çift'})"
					if shadda == 2:
						if value == valuealone:
							desc = ""
					data.append((value, desc))
	...
	for value, desc in data:
		if desc != "":
			merged[str(value)].append(desc)
		if str(value) == "361":
			merged[str(value)].append("19 sayısının karesi")
		elif str(value) == "370":
			merged[str(value)].append("Şeytan ve Lusifer kelimelerinin ebced değeri")
		elif str(value) == "480":
			merged[str(value)].append("Lilith kelimesinin ebced değeri (Ave Regina Immortalis!)")
		elif str(value) == "666":
			merged[str(value)].append("Maddi odaklılık - Denge arayışı - Canavarın sayısı (Mark of the beast)")
	
	# Tekilleştirilmiş liste oluştur
	warning_numbers = []
	for value, descs in merged.items():
		unique_descs = list(set(descs))  # Tekrar edenleri kaldır
		warning_numbers.append({
			'value': value,
			'description': "; ".join(unique_descs)
		})
	
	return warning_numbers

# Sonucu dosyaya yaz
def save_to_file(data, filename='warningNumbers.js'):
	with open(filename, 'w', encoding='utf-8') as f:
		f.write('const warningNumbers = [\n')
		for item in data:
			f.write(f'  {{ value: "{item["value"]}", description: "{item["description"]}" }},\n')
		f.write('];')

# Ana işlem
if __name__ == '__main__':
	warning_data = process_data()
	save_to_file(warning_data)
	print("warningNumbers.js oluşturuldu!")